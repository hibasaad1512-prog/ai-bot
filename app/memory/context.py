...
class ContextStore:
    ...
    def add(self,m:ChatMessage)->None:
        q=self._data.setdefault(m.chat_id,deque(maxlen=self.maxlen)); q.append(m); self._trim(m.chat_id)
    def recent(self,chat_id:int,limit:int|None=None)->list[ChatMessage]:
        self._trim(chat_id); q=list(self._data.get(chat_id,())); return q[-limit:] if limit else q
    def _trim(self,chat_id:int)->None:
        q=self._data.get(chat_id)
        if not q:return
        now=time.time()
        while q and now-q[0].timestamp>self.ttl:q.popleft()
        if not q:self._data.pop(chat_id,None)
