...
class MemoryHandlers:
    ...
    def _admin_callback(self,c):
        ...
        if d in ('memadmin:home','memadmin:back'): self._edit(c,'🔐 GOD PANEL',menu())
            ...
            with self.rt.db.engine.begin() as conn:r=conn.execute(text('DELETE FROM chat_messages WHERE chat_id=:id'),{'id':target})
            self._edit(c,f'🧹 Deleted {int(r.rowcount or 0)} stored messages.',menu())
    ...
    def _admin_media(self,c):
        self._edit(c,'🖼 Media / Telegram file IDs are stored per chat. Use Merva Lab for media actions.',menu())
    ...
    def _admin_db(self,c):
        with self.rt.db.engine.connect() as conn:
            u=conn.execute(text('SELECT COUNT(*) FROM users')).scalar() or 0; m=conn.execute(text('SELECT COUNT(*) FROM memory')).scalar() or 0; msg=conn.execute(text('SELECT COUNT(*) FROM chat_messages')).scalar() or 0
        self._edit(c,f'🗄 Neon DB OK ✅\nUsers: {u}\nMemories: {m}\nMessages: {msg}',menu())
