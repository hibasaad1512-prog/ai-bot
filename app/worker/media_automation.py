...
class MediaAutomation:
    ...
    def _target(self):
        # selected_chat_id is the persistent source of truth used by the owner panel.
        try:
    ...
    def _ensure_defaults(self,chat_id:int):
        ...
        if 'auto_media_enabled' not in s:
            s['auto_media_enabled']=True
        if 'auto_text_enabled' not in s:
            s['auto_text_enabled']=False
        s.setdefault('auto_media_interval_min',15)
        s.setdefault('auto_media_interval_max',60)
    ...
    def _send_text(self,chat_id:int)->bool:
        try:
            ...
            candidates=[m.text.strip() for m in msgs if getattr(m,'text','').strip() and not m.text.startswith('/') and len(m.text.strip())<=160]
