...
class MediaAutomation:
    """Owner-only per-group automatic media sender.

    Configuration lives in Neon chat_state. Binary files never enter PostgreSQL;
    only Telegram file IDs and metadata are persisted by ImagePool/Database.
    """
    ...
    def keyboard(self, chat_id: int):
        ...
        kb.add(types.InlineKeyboardButton("⏱ Interval", callback_data="auto:interval"), types.InlineKeyboardButton("🧹 Cleanup expired", callback_data="auto:cleanup_now"))
    ...
    def text(self, chat_id: int) -> str:
        ...
        return ("🤖 AUTO MEDIA\n\n"
                ...
                "Media is isolated per group. PostgreSQL stores only Telegram file IDs + metadata.")
    ...
    def register(self):
        ...
        def callbacks(c):
            ...
            try:
                ...
                if action == "toggle":
                    ...
                    self.save(target, auto_media_waiting_interval=True); self._edit(c, target, "⏱ Send two numbers in your private chat, e.g. `30 120` for 30–120 minutes.")
        ...
        @self.bot.message_handler(content_types=["text"], func=lambda m: is_owner(getattr(m.from_user, "id", None)) and bool(self.state(int(m.chat.id)).get("auto_media_waiting_interval")))
        def interval_input(m):
            parts=(m.text or "").split()
            try:
                if len(parts) != 2: raise ValueError
                lo=max(1,min(10080,int(parts[0]))); hi=max(lo,min(10080,int(parts[1])))
                self.save(int(m.chat.id), auto_media_interval_min=lo, auto_media_interval_max=hi, auto_media_waiting_interval=False, auto_media_next_at=0)
                self.bot.send_message(m.chat.id, self.text(self._target()) , reply_markup=self.keyboard(self._target()))
            except Exception:
                self.bot.send_message(m.chat.id, "❌ Use two numbers in minutes, e.g. `30 120`.")

    def _target(self) -> int | None:
        s=self.rt.db.get_json("chat_state", "chat_id", OWNER_ID, {})
