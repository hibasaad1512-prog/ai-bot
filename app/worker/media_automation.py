...
class MediaAutomation:
    ...
    def _ensure_defaults(self, chat_id: int):
        s = self.state(chat_id)
        d = self._global_defaults()
        s.setdefault('auto_media_enabled', d['auto_media_enabled'])
        s.setdefault('auto_text_enabled', False)
        s.setdefault('auto_media_interval_min', d['auto_media_interval_min'])
        s.setdefault('auto_media_interval_max', d['auto_media_interval_max'])
        s.setdefault('auto_media_delete_after_send', d['auto_media_delete_after_send'])
        s.setdefault('media_requests_enabled', d['media_requests_enabled'])
        s.setdefault('auto_media_next_at', 0)
        s.setdefault('auto_text_next_at', 0)
        s.setdefault('active_media_enabled', d['active_media_enabled'])
    ...
    def _group_keyboard(self, chat_id: int):
        ...
        req = bool(s.get('media_requests_enabled', True))
        ...
        kb.add(types.InlineKeyboardButton(f'📩 Requests: {"ON" if req else "OFF"}', callback_data=f'auto:requests:{chat_id}'))
        ...
        def callbacks(c):
            try:
                ...
                if not self._is_group_admin(fake):
                    self.bot.answer_callback_query(c.id, 'فقط أدمن الكروب يقدر يتحكم.', show_alert=True); return
                if int(getattr(getattr(c.message, 'chat', None), 'id', 0)) != chat_id:
                    self.bot.answer_callback_query(c.id, 'Invalid group.', show_alert=True); return
                if action == 'toggle': self.save(chat_id, auto_media_enabled=not bool(self.state(chat_id).get('auto_media_enabled', True)), auto_media_next_at=0)
                    ...
                    n = self.rt.db.delete_used_media(chat_id); self._edit(c, chat_id, f'🧹 Removed {n} used media records.'); return
    ...
    def _active_pulse(self, chat_id: int, s: dict, now: float) -> bool:
        ...
        if not self._human_recent(chat_id, 300) or now < float(s.get('active_media_next_at', 0) or 0): return False
        ...
        try:
            ...
            self.save(chat_id, active_media_next_at=now + random.randint(lo * 60, hi * 60)); return True
