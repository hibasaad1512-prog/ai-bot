...
def env_ids(name: str) -> frozenset[int]:
    ...
    for raw in os.getenv(name,"").split(","):
        try:
            if raw.strip(): out.add(int(raw.strip()))
        except ValueError: pass
    return frozenset(out)

def env_keys() -> tuple[str,...]:
    values=[]; raw=os.getenv("GROQ_API_KEYS","")
    for i,v in enumerate(raw.split(","),1):
        v=v.strip()
        if v: values.append((i,v)); os.environ.setdefault(f"GROQ_API_KEY_{i}",v)
    for name,value in os.environ.items():
        if name.startswith("GROQ_API_KEY_") and name.removeprefix("GROQ_API_KEY_").isdigit() and value.strip(): values.append((int(name.rsplit("_",1)[1]),value.strip()))
    seen={};
    for i,v in values: seen.setdefault(v,i)
...
@dataclass(frozen=True)
class Settings:
    telegram_bot_token:str=os.getenv("TELEGRAM_BOT_TOKEN","").strip(); groq_api_key:str=os.getenv("GROQ_API_KEY","").strip(); groq_api_keys:tuple[str,...]=field(default_factory=env_keys); groq_text_model:str=os.getenv("GROQ_TEXT_MODEL","openai/gpt-oss-120b").strip()
    openai_api_key:str=os.getenv("OPENAI_API_KEY","").strip(); openai_model:str=os.getenv("OPENAI_MODEL","gpt-4o-mini").strip(); deepseek_api_key:str=os.getenv("DEEPSEEK_API_KEY","").strip(); deepseek_model:str=os.getenv("DEEPSEEK_MODEL","deepseek-chat").strip(); openrouter_api_key:str=os.getenv("OPENROUTER_API_KEY","").strip(); openrouter_model:str=os.getenv("OPENROUTER_MODEL","openai/gpt-4o-mini").strip(); together_api_key:str=os.getenv("TOGETHER_API_KEY","").strip(); together_model:str=os.getenv("TOGETHER_MODEL","meta-llama/Llama-3.3-70B-Instruct-Turbo").strip(); gemini_api_key:str=os.getenv("GEMINI_API_KEY","").strip(); text_model:str=os.getenv("GEMINI_TEXT_MODEL","gemini-2.5-flash").strip(); image_model:str=os.getenv("GEMINI_IMAGE_MODEL","gemini-2.5-flash-image").strip(); ai_provider_order:str=os.getenv("AI_PROVIDER_ORDER","groq,openai,deepseek,openrouter,together,gemini").strip(); groq_admin_ids:frozenset[int]=field(default_factory=lambda:env_ids("GROQ_ADMIN_IDS")); database_url:str=os.getenv("DATABASE_URL","").strip(); redis_url:str=os.getenv("REDIS_URL","").strip(); public_base_url:str=(os.getenv("PUBLIC_BASE_URL","").strip().rstrip("/") or os.getenv("RENDER_EXTERNAL_URL","").strip().rstrip("/")); webhook_secret:str=os.getenv("WEBHOOK_SECRET","").strip()
    memory_size:int=env_int("CHAT_MEMORY_SIZE",24,10); memory_ttl_seconds:int=env_int("CHAT_MEMORY_TTL_SECONDS",7200,60); image_pool_ttl_seconds:int=env_int("IMAGE_POOL_TTL_SECONDS",21600,300); min_cooldown_seconds:int=env_int("MIN_COOLDOWN_SECONDS",20,1); max_cooldown_seconds:int=env_int("MAX_COOLDOWN_SECONDS",45,1); soft_hourly_limit:int=env_int("SOFT_HOURLY_LIMIT",20,1); hard_hourly_limit:int=env_int("HARD_HOURLY_LIMIT",30,1); max_consecutive_bot_messages:int=env_int("MAX_CONSECUTIVE_BOT_MESSAGES",2,1); max_action_payload_chars:int=env_int("MAX_ACTION_PAYLOAD_CHARS",1200,100); max_image_mb:int=env_int("MAX_IMAGE_MB",8,1)
    enabled_chaos:bool=env_bool("ENABLED_CHAOS",True); enabled_moderation:bool=env_bool("ENABLED_MODERATION",True); enabled_games:bool=env_bool("ENABLED_GAMES",True); enabled_proactive:bool=env_bool("ENABLED_PROACTIVE",True); proactive_min_interval:int=env_int("PROACTIVE_MIN_INTERVAL",7200,60); proactive_max_interval:int=env_int("PROACTIVE_MAX_INTERVAL",18000,60); proactive_chance:float=env_float("PROACTIVE_CHANCE",0.35,0,1); proactive_min_messages:int=env_int("PROACTIVE_MIN_MESSAGES",12,1); proactive_max_messages:int=env_int("PROACTIVE_MAX_MESSAGES",28,1); proactive_idle_seconds:int=env_int("PROACTIVE_IDLE_SECONDS",600,60); proactive_quiet_start:int=env_int("PROACTIVE_QUIET_START",0,0); proactive_quiet_end:int=env_int("PROACTIVE_QUIET_END",6,0); proactive_rest_minutes:int=env_int("PROACTIVE_REST_MINUTES",12,1); proactive_rest_every_hours:int=env_int("PROACTIVE_REST_EVERY_HOURS",6,1)
    reply_chance:float=env_float("REPLY_CHANCE",0.80,0,1); ai_min_score:int=env_int("AI_MIN_SCORE",34,0); callback_min_age_seconds:int=env_int("CALLBACK_MIN_AGE_SECONDS",300,60); proactive_quiet_seconds:int=env_int("PROACTIVE_QUIET_SECONDS",600,60); log_level:str=os.getenv("LOG_LEVEL","INFO").upper(); companion_bot_tokens:tuple[str,...]=field(default_factory=lambda:tuple(x.strip() for x in os.getenv("COMPANION_BOT_TOKENS","").split(",") if x.strip())); defaults:PersonalityDefaults=field(default_factory=PersonalityDefaults)
