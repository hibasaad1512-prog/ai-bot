...
class Settings:
    ...
    together_model: str = os.getenv("TOGETHER_MODEL", "meta-llama/Llama-3.3-70B-Instruct-Turbo").strip()
    ...
    reply_delay_min: float = float(os.getenv("REPLY_DELAY_MIN", "0"))
    reply_delay_max: float = float(os.getenv("REPLY_DELAY_MAX", "0"))
    same_user_limit: int = env_int("SAME_USER_LIMIT", 0, 0)
    same_user_cooldown: int = env_int("SAME_USER_COOLDOWN", 0, 0)
