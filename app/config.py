...
class Settings:
 ...
 enabled_chaos:bool=env_bool("ENABLED_CHAOS",True); enabled_moderation:bool=env_bool("ENABLED_MODERATION",True); enabled_games:bool=env_bool("ENABLED_GAMES",True); enabled_proactive:bool=env_bool("ENABLED_PROACTIVE",True); proactive_min_interval:int=env_int("PROACTIVE_MIN_INTERVAL",14400,60); proactive_max_interval:int=env_int("PROACTIVE_MAX_INTERVAL",43200,60); proactive_chance:float=env_float("PROACTIVE_CHANCE",0.55,0,1); proactive_min_messages:int=env_int("PROACTIVE_MIN_MESSAGES",30,1); proactive_max_messages:int=env_int("PROACTIVE_MAX_MESSAGES",90,1); proactive_quiet_start:int=env_int("PROACTIVE_QUIET_START",0,0); proactive_quiet_end:int=env_int("PROACTIVE_QUIET_END",6,0); proactive_rest_minutes:int=env_int("PROACTIVE_REST_MINUTES",12,1); proactive_rest_every_hours:int=env_int("PROACTIVE_REST_EVERY_HOURS",6,1)
 reply_chance:float=env_float("REPLY_CHANCE",0.80,0,1); ai_min_score:int=env_int("AI_MIN_SCORE",34,0); callback_min_age_seconds:int=env_int("CALLBACK_MIN_AGE_SECONDS",300,60); proactive_quiet_seconds:int=env_int("PROACTIVE_QUIET_SECONDS",0,0); log_level:str=os.getenv("LOG_LEVEL","INFO").upper(); companion_bot_tokens:tuple[str,...]=field(default_factory=lambda:tuple(x.strip() for x in os.getenv("COMPANION_BOT_TOKENS","").split(",") if x.strip())); defaults:PersonalityDefaults=field(default_factory=PersonalityDefaults)
 def validate(self):
