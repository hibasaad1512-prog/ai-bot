...
class Runtime:
    ...
    def _merge_dict(base,override):
        merged=dict(base or {}); merged.update({k:v for k,v in (override or {}).items() if v is not None}); return merged
    def global_personality(self):
        return Personality.from_dict(self.db.get_json("chat_settings","chat_id",0,{}).get("personality",{}))
    ...
    def _resting_now(self):
        h=datetime.now(ZoneInfo("Africa/Casablanca")).hour
        start=int(settings.proactive_quiet_start)%24; end=int(settings.proactive_quiet_end)%24
    ...
    def schedule_proactive(self,chat_id,force=False):
        ...
        low,high=self.proactive_min_seconds,self.proactive_max_seconds; nxt=now+random.randint(low,high); self.next_proactive[chat_id]=nxt; self.last_proactive.setdefault(chat_id,now); self._proactive_target_messages[chat_id]=random.randint(settings.proactive_min_messages,settings.proactive_max_messages); self._save_proactive_state(chat_id); return nxt
    def proactive_due(self,chat_id):
        ...
        if self._proactive_messages.get(chat_id,0)>=max(1,self._proactive_target_messages.get(chat_id,settings.proactive_min_messages)):return True
