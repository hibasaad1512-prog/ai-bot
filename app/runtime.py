...
class Runtime:
    def __init__(self):
        ...
        # In-memory cache backed by Neon Postgres. Messages survive
        # Render restarts and are reloaded when a chat is used again.
        ...
        self.proactive_min_seconds = int(
            getattr(settings, "proactive_min_interval", 21600)
        )
        self.proactive_max_seconds = int(
            getattr(settings, "proactive_max_interval", 54000)
        )
    ...
    def save_personality(self, chat_id: int, p: Personality):
        data = self.db.get_json(
            "chat_settings", "chat_id", chat_id, {"personality": {}}
        )
    ...
    def mark_proactive_done(self, chat_id: int) -> None:
        ...
        self.next_proactive[chat_id] = now + delay
    ...
    def proactive_remaining(self, chat_id: int) -> int:
        ...
        return max(
            0,
            int(self.next_proactive[chat_id] - time.time()),
        )
