...
def menu():
 ...
 k.add(types.InlineKeyboardButton('🚪 خروج من المختبر','mad:exit'));return k
def group_menu(gs):
 from telebot import types
...
def register(bot,runtime):
 ...
 def cb(c):
  ...
  try:
   ...
   if d in ('mad:open','mad:exit'):
    ...
    bot.send_message(c.message.chat.id,'🚪 تم الخروج من مختبر الميرفاوية.\n\n🔐 GOD PANEL',reply_markup=menu());return
   ...
   if d=='mad:auto':
    enabled=not bool(state(runtime.db).get('mad_auto'));save(runtime.db,mad_auto=enabled)
    bot.send_message(c.message.chat.id,('🤖 Auto Send: ON\n💾 الحالة محفوظة في Neon وتبقى بعد Restart.' if enabled else '🛑 Auto Send: OFF'),reply_markup=menu());return
   if d=='mad:autoplus':
    s=state(runtime.db);enabled=not bool(s.get('mad_autoplus'));save(runtime.db,mad_autoplus=enabled,mad_auto=enabled)
    bot.send_message(c.message.chat.id,('🔥 Auto+ ON\n🎲 يختار عشوائيًا بين media/remix/poll/mood عند تشغيل الـworker.\n💾 محفوظة في Neon.' if enabled else '🛑 Auto+ OFF'),reply_markup=menu());return
   t=target(runtime.db)
   if not t:return bot.send_message(c.message.chat.id,'🎯 اختار كروبًا أولًا.')
   msgs=runtime.db.recent_messages(t,150)
   if d=='mad:send':save(runtime.db,mad_waiting='send');return bot.send_message(c.message.chat.id,'📨 أرسل الآن أي رسالة: نص، صورة، فيديو، Sticker، GIF أو ملف.')
   if d=='mad:random' and msgs:bot.copy_message(t,t,random.choice(msgs).message_id)
    ...
    k=__import__('telebot').types.InlineKeyboardMarkup();k.add(__import__('telebot').types.InlineKeyboardButton(f'⭐ Tip {random.randint(1,1000)}',url=f'https://t.me/{TIP_USERNAME}'))
    bot.send_message(t,'🎁 دعم الميرفاوية',reply_markup=k)
 ...
 def lab_input(m):
  ...
  if mode=='addchat':
   if getattr(m,'forward_from_chat',None):remember={'chat_id':m.forward_from_chat.id,'title':getattr(m.forward_from_chat,'title',None),'username':getattr(m.forward_from_chat,'username',None)};s=state(runtime.db);known=[x for x in s.get('known_chats',[]) if int(x.get('chat_id',0))!=int(m.forward_from_chat.id)];known.insert(0,remember);save(runtime.db,known_chats=known[:100],chaos_target_chat_id=m.forward_from_chat.id,mad_waiting=False);bot.send_message(m.chat.id,'✅ تم اختيار الكروب.',reply_markup=menu());return
