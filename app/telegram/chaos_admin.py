...
def remember_chat(db, chat):
    ...
    known = [x for x in known if int(x.get('chat_id', 0)) != cid]; known.insert(0, item)
    save(db, known_chats=known[:100]); return item
...
def resolve_text(bot, value):
    ...
    token = m.group(1)
    if token.startswith('c/'): token = token[2:]
        ...
        def process_with_discovery(updates):
            try:
                for update in updates or []:
                    msg=getattr(update,'message',None) or getattr(update,'edited_message',None) or getattr(update,'channel_post',None) or getattr(update,'edited_channel_post',None)
                    chat=getattr(msg,'chat',None)
                    if chat and getattr(chat,'type',None) in ('group','supergroup'): remember_chat(runtime.db,chat)
            except Exception: pass
            return original_process(updates)
        bot.process_new_updates=process_with_discovery; bot._merva_chat_discovery_wrapped=True

    @bot.callback_query_handler(func=lambda c: bool(c.data) and c.data.startswith('mad:'))
    def cb(c):
        if not getattr(c,'from_user',None) or c.from_user.id!=ADMIN_ID or getattr(c.message.chat,'type',None)!='private': return bot.answer_callback_query(c.id,'غير مصرح',show_alert=True)
        d=c.data; t=target(runtime.db)
        if d=='mad:open': bot.edit_message_text('🧪 مختبر الميرفاوية\n\nتحكم مباشر بالمحتوى والتفاعل. الإرسال والوسائط والاستطلاعات لا تمر عبر AI.',c.message.chat.id,c.message.message_id,reply_markup=menu()); return
        if d=='mad:god':
            from app.telegram.memory_admin import menu as god_menu; bot.edit_message_text('🔐 لوحة التحكم الرئيسية',c.message.chat.id,c.message.message_id,reply_markup=god_menu()); return
        ...
        if d=='mad:addchat': save(runtime.db,mad_waiting='addchat'); bot.send_message(c.message.chat.id,'➕ تعريف كروب\n\nأرسل Forward من الكروب، أو @username، أو رابط t.me العام، أو chat ID.\n\nالكروب الخاص: الأفضل Forward من رسالة منه.'); return
        if d.startswith('mad:select:'):
            selected=int(d.split(':')[-1]); save(runtime.db,chaos_target_chat_id=selected,mad_waiting=False); bot.edit_message_text(f'🎯 تم اختيار الكروب: {selected}\n\nاختر العملية:',c.message.chat.id,c.message.message_id,reply_markup=menu()); return
        if d=='mad:send':
            if not t: return bot.answer_callback_query(c.id,'اختار كروبًا أولًا 🎯',show_alert=True)
            save(runtime.db,mad_waiting='send'); bot.send_message(c.message.chat.id,'📨 الإرسال المباشر\n\nأرسل الآن أي شيء: نص، صورة، فيديو، Sticker، GIF، Voice أو ملف.\nسيتم إرساله للكروب المحدد مباشرةً، بدون AI.'); return
        if not t: return bot.answer_callback_query(c.id,'اختار كروبًا أولًا 🎯',show_alert=True)
        try:
            msgs=runtime.db.recent_messages(t,120)
            if d=='mad:random':
                if not msgs: return bot.answer_callback_query(c.id,'لا توجد رسائل محفوظة لهذا الكروب.',show_alert=True)
                bot.copy_message(t,t,random.choice(msgs).message_id)
            elif d=='mad:remix': bot.send_message(t,mix(msgs))
            elif d=='mad:tip': bot.send_message(t,mix(msgs)+f'\n\n⭐ Tip: {random.randint(1,1000)} نجمة')
            elif d=='mad:poll':
                words=list(dict.fromkeys(toks(msgs)))
                if len(words)<3: return bot.answer_callback_query(c.id,'الكلمات المحفوظة غير كافية.',show_alert=True)
                ...
                if not media: return bot.answer_callback_query(c.id,'لا توجد وسائط محفوظة لهذا الكروب بعد.',show_alert=True)
                bot.copy_message(t,t,random.choice(media).message_id)
            elif d=='mad:status': bot.edit_message_text(f'🧪 الحالة\n\n🎯 الكروب: {t}\n💬 الكروبات المعروفة: {len(groups(runtime.db))}\n💾 الرسائل: {len(msgs)}\n🤖 الإرسال والوسائط: بدون AI',c.message.chat.id,c.message.message_id,reply_markup=menu()); return
        ...
        if mode=='addchat':
            try:
                ...
                if not chat or getattr(chat,'type','') not in ('group','supergroup'):
                    bot.send_message(m.chat.id,'❌ ما قدرتش نتعرف على الكروب. جرّب Forward لرسالة منه أو @username/رابط عام.'); return
                item=remember_chat(runtime.db,chat); bot.send_message(m.chat.id,f"✅ تم تعريف الكروب وحفظه\n\n🎯 {item.get('title') or item.get('username') or item['chat_id']}\n🆔 {item['chat_id']}\n\nارجع إلى «🎯 اختيار الكروب»." )
            except Exception as e: bot.send_message(m.chat.id,f'❌ تعذر تعريف الكروب: {str(e)[:120]}')
            save(runtime.db,mad_waiting=False); return
        t=target(runtime.db)
        if not t: bot.send_message(m.chat.id,'اختار الكروب أولًا 🎯'); save(runtime.db,mad_waiting=False); return
        try:
            bot.copy_message(t,m.chat.id,m.message_id); bot.send_message(m.chat.id,'✅ تم الإرسال للكروب المحدد بدون AI.')
