...
class TelegramHandlers:
    ...
    def _random_callback_context(
        ...
        selected = random.choice(candidates[-25:])
        context = PrivacyFilter.non_verbatim(selected.text)
        if not context:
            return ""

        return (
            "\n\nRANDOM OLDER MESSAGE (NON-VERBATIM):\n"
            f"User: {context}\n"
            "You may react to this older topic only if it naturally fits. "
            "Do not quote or reconstruct the original message."
        )
    ...
    def _random_remix_context(
        ...
        candidates = [
            x for x in recent
            if x.text and not x.is_bot and len(x.text.strip()) >= 2
        ]
        if len(candidates) < 3:
            return ""
        ...
        for message in chosen:
            hint = PrivacyFilter.non_verbatim(message.text, max_terms=5)
            if hint:
                hints.append(hint)
        ...
        return (
            "\n\nRANDOM CHAT HINTS (NON-VERBATIM):\n"
            + "\n".join(hints)
            + "\n\nUse these only as loose topic hints. "
            "Do not quote, stitch together, or reconstruct source messages."
        )
    ...
    def _conversation_context(
        ...
        for message in recent:
            ...
            speaker = (
                self._bot_username
                if message.is_bot
                else PrivacyFilter.anonymized_speaker(message.user_id)
            )

            safe_context = PrivacyFilter.non_verbatim(message.text)
            if not safe_context:
                continue
            lines.append(
                f"{speaker}: {safe_context}"
            )
        ...
        direct = (
            ...
            f"CURRENT MESSAGE: {PrivacyFilter.sanitize(current_text).text[:1000]}\n"
        )
        ...
        if (
            ...
            reply_hint = PrivacyFilter.non_verbatim(m.reply_to_message.text)
            if reply_hint:
                direct += (
                    "\nMESSAGE BEING REPLIED TO (NON-VERBATIM):\n"
                    f"{reply_hint}\n"
                )
...
"""
    ...
    def on_message(self, m):
        ...
        # Learn human text for this specific group/user.
        if safe_text and not privacy.redacted:
            try:
        ...
        # Hard privacy boundary: sensitive or credential-bearing messages
        # are not forwarded to the external AI provider.
        ...
        try:
            ...
            learning_summary = ""
            try:
                learning_summary = self.rt.learning.prompt_summary(
                    m.chat.id,
                    m.from_user.id,
                )
            except Exception:
    ...
    def proactive(
        ...
        for item in recent[-20:]:
            hint = PrivacyFilter.non_verbatim(item.text) if item.text else ""
            if hint:
                context_rows.append(f"User: {hint}")
        context = "\n".join(context_rows)
        ...
        if (
            ...
            hint = PrivacyFilter.non_verbatim(selected.text)
            if hint:
                context += (
                    "\n\nRANDOM OLDER MESSAGE (NON-VERBATIM):\n"
                    f"User: {hint}"
        ...
        try:
            ...
            self.rt.memory.add(
                ChatMessage(
                    ...
                    display_name=self._bot_username,
                    timestamp=time.time(),
                    text=PrivacyFilter.sanitize(txt[:1000]).text,
